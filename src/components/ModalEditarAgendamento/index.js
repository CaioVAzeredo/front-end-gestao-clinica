...
function ModalEditarAgendamento({ open, initialData, servicos = [], funcionarios = [], onClose, onSubmit }) {
    ...
    // Essa condição foi mantida para evitar a renderização do conteúdo do modal
    // quando ele está fechado, mas a Overlay continua sendo renderizada para a transição.
    if (!open && !initialData) {
        return null;
    }
    ...
    return (
        <ModalOverlay onMouseDown={onOverlayClick} isOpen={open}>
